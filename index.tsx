...
// --- API Helper ---
const generateGeminiResponse = async (apiKey: string, history: any[], newPrompt: string) => {
  ...
  const systemInstruction = {
     parts: [{ text: "You are a super cute, supportive, and kawaii travel fund assistant. The user is trying to save money by doing 5-minute work sprints. When the user complains or whines (sajiao), be very sympathetic, use emojis (like ✨, 💖, (´｡• ᵕ •｡`)), but gently encourage them to just do 5 minutes. Remind them of the travel goal. Keep responses short (under 50 words) and sweet. Act like a close friend." }]
  };
...
const PixelCard = ({ children, className = "", title, icon }: any) => (
...
const PixelButton = ({ children, variant = 'primary', className = "", ...props }: any) => {
...
const ChatBubble = ({ role, text }: any) => {
    ...
    const htmlContent = (window as any).marked ? (window as any).marked.parse(text) : text;
    ...
    return (
        <div className={`flex w-full mb-2 ${role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            >
               {role === 'model' ? <div dangerouslySetInnerHTML={{ __html: htmlContent }} /> : text}
            </div>
        </div>
    );
};
...
function App() {
  ...
  const [chatMessages, setChatMessages] = useState([{role: 'model', text: "Hii! I'm here if you feel tired! (´｡• ᵕ •｡`) ♡"}]);
  ...
  const addStandardRecord = (type: string, amount: number, description: string) => {
    const newRecord = {
  ...
  // Timer Interval (Corrected Logic: Time Delta)
  useEffect(() => {
    if (isTimerRunning && sessionStartTime) {
      ...
      }, 500); // Check every 0.5s to keep UI snappy, even if logic relies on full seconds
    } else {
      if (timerRef.current) clearInterval(timerRef.current);
    }
    return () => {
  ...
  }, [isTimerRunning, sessionStartTime, finishTimer]);
  ...
  const handleChatSend = async (e: any) => {
     ...
     if (!apiKey) {
         setChatMessages(prev => [...prev, {role: 'model', text: "Wait! I need your Gemini API Key first. Click the Gear icon (⚙️) to set it up! (｡•́︿•̀｡)"}]);
         setChatInput("");
         return;
     }
    ...
    <div className="min-h-screen p-4 md:p-8 max-w-lg mx-auto flex flex-col gap-6 text-lg relative pb-32 font-['VT323']">
      ...
      <div className="absolute top-4 right-4 z-20 flex gap-2">
         <button 
          ...
          className="bg-white border-2 border-gray-900 text-gray-900 p-2 rounded-lg shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:bg-gray-100 active:translate-y-1 active:shadow-none transition-all"
          title="Settings"
        >
          <Icons.Settings />
        </button>
           ...
           <div className="mb-4 w-80 bg-white border-4 border-gray-900 rounded-xl shadow-[8px_8px_0px_0px_rgba(0,0,0,0.2)] animate-bounce-in flex flex-col overflow-hidden max-h-[500px]">
              <div className="bg-pink-300 p-3 border-b-4 border-gray-900 flex justify-between items-center">
                 <div className="font-bold text-gray-900 flex items-center gap-2">
                     <span className="text-2xl">💖</span> Comfort Corner
                 </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border-4 border-gray-900 shadow-[8px_8px_0px_0px_rgba(255,255,255,0.5)] rounded-2xl p-6 w-full max-w-sm animate-bounce-in font-['VT323']">
            ...
            {activeModal === 'settings' && (
              <div className="">
                <div className="flex justify-between items-center mb-4">
                   <h3 className="text-3xl font-bold">⚙️ Settings</h3>
                   <button onClick={() => setActiveModal(null)}><Icons.X /></button>
                </div>
